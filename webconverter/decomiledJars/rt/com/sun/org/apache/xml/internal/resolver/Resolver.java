  ...
  extends Catalog
{
  ...
  protected Resolver queryResolver(String paramString1, String paramString2, String paramString3, String paramString4)
  {
    ...
    catch (CatalogException localCatalogException)
    {
      if (localCatalogException.getExceptionType() == 6) {
        this.catalogManager.debug.message(1, "Unparseable catalog: " + str1);
      } else if (localCatalogException.getExceptionType() == 5) {
        this.catalogManager.debug.message(1, "Unknown catalog format: " + str1);
      }
      return null;
    }
    catch (MalformedURLException localMalformedURLException)
    {
      this.catalogManager.debug.message(1, "Malformed resolver URL: " + str1);
      return null;
    }
    catch (IOException localIOException)
    {
      this.catalogManager.debug.message(1, "I/O Exception opening resolver: " + str1);
    }
    return null;
  }
    ...
    throws MalformedURLException, IOException
  {
    ...
    for (int i = 0; i < this.catalogs.size(); i++)
    {
      ...
      try
      {
        localResolver = (Resolver)this.catalogs.elementAt(i);
      }
      catch (ClassCastException localClassCastException)
      {
        localObject = (String)this.catalogs.elementAt(i);
        ...
        {
          this.catalogManager.debug.message(1, "Malformed Catalog URL", (String)localObject);
        }
        catch (FileNotFoundException localFileNotFoundException)
        {
          this.catalogManager.debug.message(1, "Failed to load catalog, file not found", (String)localObject);
        }
        catch (IOException localIOException)
        {
          this.catalogManager.debug.message(1, "Failed to load catalog, I/O error", (String)localObject);
        }
        this.catalogs.setElementAt(localResolver, i);
      }
      String str = null;
      if (paramInt == DOCTYPE)
      {
