  ...
  extends RECompiler
{
  ...
  {
    char c = this.instruction[(paramInt + 0)];
    int i = this.instruction[(paramInt + 1)];
  ...
  public void dumpProgram(PrintWriter paramPrintWriter)
  {
    ...
    {
      int j = this.instruction[(i + 0)];
      int k = this.instruction[(i + 1)];
      int m = (short)this.instruction[(i + 2)];
      ...
      if (j == 65)
      {
        ...
        n = k;
        while (n-- != 0) {
          paramPrintWriter.print(charToString(this.instruction[(i++)]));
        }
        paramPrintWriter.print("\"");
      }
